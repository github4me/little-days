...
- 提醒在「我的」设置。喂养可选「随最新喂养」：每次保存、编辑、删除或恢复记录后，应用以开始时间最新的一次喂养加上设定间隔重新安排提醒；也可选仅提醒一次或每天固定时间。免打扰/专注模式和系统通知权限可能影响呈现。没有自动作息建议。
...
这是 0.1.0 开发交付版，不是已签名安装包。没有家庭共享、照片、后端、锁屏组件。需要 iPhone 实测通知、原生日期选择、SQLite 重启保存、文件选择/导出，以及最终签名安装流程。
